...
Interactive controls (buttons and text inputs) SHALL share a single height and horizontal padding so they align when placed together, regardless of borders.

#### Scenario: Button aligns with adjacent input

- **WHEN** a button sits next to a text input (e.g. `+ New` beside search)
- **THEN** their heights match and their edges align

#### Scenario: Defined button variants

- **WHEN** a button is rendered
- **THEN** it uses one of the defined variants (primary/secondary/ghost/danger/icon) with consistent sizing
